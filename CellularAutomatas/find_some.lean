...
lemma find_some_bounded_idx_eq_find_some_idxd_of_repeating_function { α } { f: ℕ → Option α } (h: RepeatingFunction f): find_some_bounded_idx f h.k = find_some_idxd f := by
